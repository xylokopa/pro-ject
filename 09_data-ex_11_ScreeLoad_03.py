...
df_original = pd.read_csv("Auto-tabloid.csv").rename(columns={'name': 'ID_Number'})

# 1. Entfernt Klammern und den Inhalt darin
df_original['name'] = df_original['name'].str.replace(r'\s*\(.*?\)', '', regex=True)

# 2. Kürzt den Namen auf maximal 15 Zeichen und entfernt Leerzeichen am Ende
df_original['name'] = df_original['name'].str.slice(0, 15).str.strip()
...
def update():
    ...
    # Links: Die Orgel-Balken der Lambdas
    ...
    ax2.set_title("Duftiges Aqua-Ballett")
